...
class ChicagoCrimeTool(BaseTool):
    ...
    def __init__(self, app_token: Optional[str] = None):
        ...
        self.client.session.timeout = self.timeout_seconds
    ...
    def _run(
        ...
        try:
            ...
            try:
                ...
                query_time = time.time() - start_time
                error_msg = f"Chicago API timeout after {query_time:.2f} seconds (timeout #{self.timeout_count}). Skipping LLM processing to avoid incomplete data."
                print(error_msg)
                ...
                if "timeout" in str(e).lower() or "timed out" in str(e).lower():
                    self.timeout_count += 1
                    error_msg = f"Chicago API timeout after {query_time:.2f} seconds (timeout #{self.timeout_count}). Skipping LLM processing."
                    print(error_msg)
                    return f"DATABASE_TIMEOUT: {error_msg}"
                raise e
            ...
            if "timeout" in str(e).lower() or "timed out" in str(e).lower():
                self.timeout_count += 1
                return f"DATABASE_TIMEOUT: Chicago API timeout (timeout #{self.timeout_count}). Skipping LLM processing."
